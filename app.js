let data = `Gorillas can catch human colds and other illnesses.
...
let facts = data.trim().split("\n\n", 300);
for(let i=0; i< facts.length; i++){
    facts[i] = facts[i].replace(/(\r\n|\n|\r)/gm,"")
}
...
button.addEventListener('click', function(e){
    ...
    const factContent = facts[Math.floor(Math.random()*301)];

    clearDiv('fact');

    
    newFact.innerHTML = `"${factContent}"`;
